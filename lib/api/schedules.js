...
export const scheduleApi = {
  ...
  getByEmployee: async (employeeId, startDate, endDate) => {
    try {
      ...
      const response = await apiClient.get(`/schedules/employee/${employeeId}`, { params });
   ...
   * GET /schedules (with startDate and endDate query params)
   * @param {string} startDate - Start date (YYYY-MM-DD format)
   * @param {string} endDate - End date (YYYY-MM-DD format)
  ...
  getByDateRange: async (startDate, endDate, params = {}) => {
    try {
      // Backend uses GET /schedules with startDate, endDate query params
  ...
  /**
   * Mark schedule as unavailable (request time off)
   * PUT /schedules/:id/unavailable
   * @param {number} scheduleId - Schedule ID
   * @param {string} [reason] - Optional reason for unavailability
   * @returns {Promise<{success: boolean, data?: ScheduleResponse, message?: string, error?: string}>}
   */
  markUnavailable: async (scheduleId, reason = '') => {
    try {
      const response = await apiClient.put(`/schedules/${scheduleId}/unavailable`, { reason });

      return {
        success: true,
        data: response.data,
        message: 'Đã đánh dấu không nhận lịch',
      };
    } catch (error) {
      return handleApiError(error);
    }
   ...
   * PUT /schedules/:id/available
   * @param {number} scheduleId - Schedule ID
   * @returns {Promise<{success: boolean, data?: ScheduleResponse, message?: string, error?: string}>}
   */
  markAvailable: async (scheduleId) => {
    try {
      const response = await apiClient.put(`/schedules/${scheduleId}/available`);

      return {
        success: true,
        data: response.data,
        message: 'Đã đánh dấu sẵn sàng nhận lịch',
      };
    } catch (error) {
      return handleApiError(error);
    }
