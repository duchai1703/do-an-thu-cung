...
export const appointmentApi = {
  ...
  /**
   * Create appointment for current user (pet owner)
   * POST /appointments/me
   * @param {CreateAppointmentDto} appointmentData - Appointment data (petId, employeeId, serviceId, appointmentDate, startTime, endTime)
   * @returns {Promise<{success: boolean, data?: AppointmentResponse, message?: string, error?: string}>}
   */
  createMyAppointment: async (appointmentData) => {
    try {
      const response = await apiClient.post('/appointments/me', appointmentData);
   ...
   * GET /appointments/me
   * @param {AppointmentStatus} [status] - Optional status filter
   * @returns {Promise<{success: boolean, data?: AppointmentResponse[], error?: string}>}
   */
  getMyAppointments: async (status) => {
    try {
      const params = status ? { status } : {};
      const response = await apiClient.get('/appointments/me', { params });
