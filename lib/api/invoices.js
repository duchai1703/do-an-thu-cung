 ...
 * @typedef {import('./types').InvoiceStatus} InvoiceStatus
 * @typedef {import('./types').GetInvoicesQuery} GetInvoicesQuery
 */
...
export const invoiceApi = {
   ...
   * @param {Object} data - {appointmentId, discountCode?, notes?}
   * @returns {Promise<{success: boolean, data?: InvoiceResponse, message?: string, error?: string}>}
   */
  generate: async (data) => {
    try {
  ...
  /**
   * Get current logged-in pet owner's invoices
   * GET /invoices/me
   * @param {GetInvoicesQuery} [params={}] - Query parameters for filtering (status, startDate, endDate, includeAppointment, includePet)
   * @returns {Promise<{success: boolean, data?: InvoiceResponse[], error?: string}>}
   */
  getMyInvoices: async (params = {}) => {
    try {
      console.log('Fetching my invoices with params:', params);
      const response = await apiClient.get('/invoices/me', { params });
   ...
   * Get all invoices
   * GET /invoices
   * @param {GetInvoicesQuery} [params={}] - Query parameters for filtering (status, startDate, endDate)
   * @returns {Promise<{success: boolean, data?: InvoiceResponse[], error?: string}>}
   */
  getAll: async (params = {}) => {
    try {
   ...
   * @returns {Promise<{success: boolean, data?: InvoiceResponse, error?: string}>}
   */
  getById: async (invoiceId, params = {}) => {
    try {
      const response = await apiClient.get(`/invoices/${invoiceId}`, { params });
   ...
   * GET /invoices/statistics/by-customer
   * Returns statistics for all customers: totalVisits, totalSpent, lastVisit
   * @returns {Promise<{success: boolean, data?: Array<{petOwnerId: number, totalVisits: number, totalSpent: number, lastVisit: Date|null}>, error?: string}>}
   */
  getCustomerStatistics: async () => {
    try {
      const response = await apiClient.get('/invoices/statistics/by-customer');
